...
from .brainco_reorient_state import (
  advance_goal_yaw_wxyz,
  get_goal_rot_wxyz,
  reset_goal_rot_wxyz,
)
...
def actuator_work_l2_penalty(
  ...
  By default this uses tau*qdot. Set ``use_joint_acc=True`` to use tau*qacc
  as a finite-difference proxy.
...
class object_yaw_finite_diff_clipped:
  ...
  def __call__(
    ...
    if torch.any(missing):
      self._init_pos_w[missing] = cube.data.root_link_pos_w[missing]
...
def object_linvel_l1(
  ...
  """Object linear velocity penalty term: ||v_obj||_1."""
...
def _target_pos(
  ...
  return torch.tensor(target_pos, dtype=torch.float32, device=env.device).repeat(
    env.num_envs, 1
  )


def brainco_object_goal_distance(
  env: ManagerBasedRlEnv,
  object_name: str,
  target_pos: tuple[float, float, float],
) -> torch.Tensor:
  """BrainCo direct reorient distance: ||object_pos - in_hand_pos||."""
  return torch.linalg.vector_norm(
    _object_pos_env(env, object_name) - _target_pos(env, target_pos),
    ord=2,
    dim=-1,
  )


def brainco_rotation_distance(
  env: ManagerBasedRlEnv,
  object_name: str,
  initial_goal_quat_wxyz: tuple[float, float, float, float],
) -> torch.Tensor:
  """BrainCo direct reorient quaternion distance to the current goal."""
  obj: Entity = env.scene[object_name]
  goal = get_goal_rot_wxyz(env, initial_goal_quat_wxyz)
  quat_diff = quat_mul(obj.data.root_link_quat_w, quat_conjugate(goal))
  return 2.0 * torch.asin(
    torch.clamp(torch.linalg.vector_norm(quat_diff[:, 1:4], ord=2, dim=-1), max=1.0)
  )


def brainco_rotation_reward(
  env: ManagerBasedRlEnv,
  object_name: str,
  initial_goal_quat_wxyz: tuple[float, float, float, float],
  rot_eps: float = 0.1,
) -> torch.Tensor:
  """BrainCo direct reorient rotation reward: 1 / (|rot_dist| + eps)."""
  rot_dist = brainco_rotation_distance(env, object_name, initial_goal_quat_wxyz)
  return 1.0 / (torch.abs(rot_dist) + rot_eps)


def brainco_action_l2(env: ManagerBasedRlEnv) -> torch.Tensor:
  """BrainCo direct reorient action penalty: sum(actions^2)."""
  return torch.sum(torch.square(env.action_manager.action), dim=-1)


def brainco_fall_penalty(
  env: ManagerBasedRlEnv,
  object_name: str,
  target_pos: tuple[float, float, float],
  fall_dist: float,
) -> torch.Tensor:
  """Indicator penalty for object leaving the allowed in-hand region."""
  goal_dist = brainco_object_goal_distance(env, object_name, target_pos)
  return (goal_dist >= fall_dist).float()


class brainco_reach_goal_bonus:
  """BrainCo direct reorient success bonus and target-yaw advancement."""

  def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv):
    self.object_name = cfg.params["object_name"]
    self.initial_goal_quat_wxyz = cfg.params["initial_goal_quat_wxyz"]
    self.target_pos = cfg.params.get("target_pos")
    self.success_tolerance = float(cfg.params.get("success_tolerance", 0.2))
    self.yaw_offset = float(cfg.params.get("yaw_offset", 0.39269908169872414))
    self._env = env
    self.successes = torch.zeros(env.num_envs, dtype=torch.float, device=env.device)
    reset_goal_rot_wxyz(env, None, self.initial_goal_quat_wxyz)

  def reset(self, env_ids: torch.Tensor | slice | None = None):
    reset_goal_rot_wxyz(self._env, env_ids, self.initial_goal_quat_wxyz)
    if env_ids is None:
      env_ids = slice(None)
  ...
  def __call__(
    ...
    yaw_offset: float = 0.39269908169872414,
    target_pos: tuple[float, float, float] | None = None,
  ) -> torch.Tensor:
    del object_name, initial_goal_quat_wxyz, success_tolerance, yaw_offset, target_pos
    rot_dist = brainco_rotation_distance(
      env,
      self.object_name,
      self.initial_goal_quat_wxyz,
    )
    reached = torch.abs(rot_dist) <= self.success_tolerance
    reached_float = reached.float()
    self.successes += reached_float
    log = env.extras.setdefault("log", {})
    log["rot_dist"] = torch.mean(rot_dist)
    log["success_rate"] = torch.mean(reached_float)
    log["consecutive_successes"] = torch.mean(self.successes)
    if self.target_pos is not None:
      goal_dist = brainco_object_goal_distance(env, self.object_name, self.target_pos)
      log["goal_dist"] = torch.mean(goal_dist)
...
def joint_torque_l2(
  ...
  """Joint torque penalty term: sum(tau^2)."""
  asset: Entity = env.scene[asset_cfg.name]
  tau = asset.data.actuator_force[:, asset_cfg.joint_ids]
  if max_abs_torque is not None:
    tau = torch.clamp(tau, min=-max_abs_torque, max=max_abs_torque)
