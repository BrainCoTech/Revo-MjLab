...
from .brainco_reorient_state import get_goal_rot_wxyz
...
def joint_pos_unscaled(
  env: ManagerBasedRlEnv,
  asset_cfg: SceneEntityCfg = _DEFAULT_JOINT_ASSET_CFG,
  use_soft_limits: bool = False,
) -> torch.Tensor:
  """Joint positions normalized to [-1, 1] with joint limits."""
  asset: Entity = env.scene[asset_cfg.name]
  joint_pos = asset.data.joint_pos[:, asset_cfg.joint_ids]
  limits = (
    asset.data.soft_joint_pos_limits
    if use_soft_limits
    else asset.data.joint_pos_limits
  )
  limits = limits[:, asset_cfg.joint_ids]
  lower = limits[..., 0]
  upper = limits[..., 1]
  denom = torch.clamp(upper - lower, min=1e-6)
  return (2.0 * joint_pos - upper - lower) / denom
...
def joint_vel_scaled(
  ...
  """Joint velocities multiplied by a scalar observation scale."""
  asset: Entity = env.scene[asset_cfg.name]
  return scale * asset.data.joint_vel[:, asset_cfg.joint_ids]


def action_raw(env: ManagerBasedRlEnv) -> torch.Tensor:
  """Current raw policy action stored by the action manager."""
...
def constant_vector(
  ...
  """Repeat a fixed vector for every environment."""
  return torch.tensor(value, dtype=torch.float32, device=env.device).repeat(
    env.num_envs, 1
  )
...
def object_pos_env(
  ...
  """Object root position relative to each environment origin."""
  obj: Entity = env.scene[object_name]
  return obj.data.root_link_pos_w - env.scene.env_origins


def object_quat_wxyz(
  env: ManagerBasedRlEnv,
  object_name: str,
) -> torch.Tensor:
  """Object root quaternion in MjLab/MuJoCo wxyz order."""
  obj: Entity = env.scene[object_name]
...
def object_lin_vel_w(
  ...
  obj: Entity = env.scene[object_name]
  return obj.data.root_link_lin_vel_w


def object_ang_vel_w_scaled(
  env: ManagerBasedRlEnv,
  object_name: str,
  scale: float = 1.0,
) -> torch.Tensor:
  """Object root angular velocity in world frame with scalar scaling."""
  obj: Entity = env.scene[object_name]
  return scale * obj.data.root_link_ang_vel_w


def object_goal_quat_diff_wxyz(
  env: ManagerBasedRlEnv,
  object_name: str,
  goal_quat_wxyz: tuple[float, float, float, float],
) -> torch.Tensor:
  """Object * conjugate(goal) quaternion, in wxyz order."""
  obj: Entity = env.scene[object_name]
  goal = torch.tensor(goal_quat_wxyz, dtype=torch.float32, device=env.device).repeat(
    env.num_envs, 1
  )
  return quat_mul(obj.data.root_link_quat_w, quat_conjugate(goal))


def brainco_goal_rot_wxyz(
  env: ManagerBasedRlEnv,
  initial_goal_quat_wxyz: tuple[float, float, float, float],
) -> torch.Tensor:
  """Current BrainCo reorient target orientation in wxyz order."""
  return get_goal_rot_wxyz(env, initial_goal_quat_wxyz)


def brainco_object_goal_quat_diff_wxyz(
  env: ManagerBasedRlEnv,
  object_name: str,
  initial_goal_quat_wxyz: tuple[float, float, float, float],
) -> torch.Tensor:
  """Object * conjugate(current BrainCo goal), in wxyz order."""
  obj: Entity = env.scene[object_name]
  goal = get_goal_rot_wxyz(env, initial_goal_quat_wxyz)
  return quat_mul(obj.data.root_link_quat_w, quat_conjugate(goal))


def body_pos_env_flat(
  env: ManagerBasedRlEnv,
  asset_cfg: SceneEntityCfg,
) -> torch.Tensor:
  """Selected body positions relative to env origin, flattened."""
  asset: Entity = env.scene[asset_cfg.name]
  pos = asset.data.body_link_pos_w[:, asset_cfg.body_ids]
  pos = pos - env.scene.env_origins[:, None, :]
  return pos.reshape(env.num_envs, -1)


def body_quat_wxyz_flat(
  env: ManagerBasedRlEnv,
  asset_cfg: SceneEntityCfg,
) -> torch.Tensor:
  """Selected body quaternions in wxyz order, flattened."""
  asset: Entity = env.scene[asset_cfg.name]
  return asset.data.body_link_quat_w[:, asset_cfg.body_ids].reshape(
    env.num_envs, -1
  )
...
def body_vel_w_flat(
  ...
  """Selected body velocities [linear, angular] in world frame, flattened."""
  asset: Entity = env.scene[asset_cfg.name]
  return asset.data.body_link_vel_w[:, asset_cfg.body_ids].reshape(env.num_envs, -1)
...
def palm_center_pose_w(
  ...
  if palm_geom_ids is None:
    try:
      palm_geom_ids, _ = hand.find_geoms(
