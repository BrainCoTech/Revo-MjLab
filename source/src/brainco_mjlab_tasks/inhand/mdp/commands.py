...
def _viewer_array(value, dtype=np.float64) -> np.ndarray:
...
class HandCubeFrameVizCommand(CommandTerm):
  ...
  def __init__(self, cfg: HandCubeFrameVizCommandCfg, env: ManagerBasedRlEnv):
    ...
    self._target_pos_env = torch.tensor(
  ...
  def _debug_vis_impl(self, visualizer: "DebugVisualizer") -> None:
    ...
    target_pos_w = self._env.scene.env_origins + self._target_pos_env
    ...
    for env_id in env_indices:
      if viz.show_world_frame:
        visualizer.add_frame(
          position=_viewer_array(self._env.scene.env_origins[env_id]),
          rotation_matrix=_viewer_array(self._identity_rotm),
      ...
      if viz.show_palm_body_frame:
        visualizer.add_frame(
          position=_viewer_array(palm_pos_w[env_id]),
          rotation_matrix=_viewer_array(palm_rotm_w[env_id]),
      ...
      if viz.show_palm_center_frame:
        visualizer.add_frame(
          position=_viewer_array(palm_center_pos_w[env_id]),
          rotation_matrix=_viewer_array(palm_rotm_w[env_id]),
      ...
      if viz.show_cube_frame:
        visualizer.add_frame(
          position=_viewer_array(cube_pos_w[env_id]),
          rotation_matrix=_viewer_array(cube_rotm_w[env_id]),
      ...
      if viz.show_palm_to_cube_arrow:
        visualizer.add_arrow(
          start=_viewer_array(palm_center_pos_w[env_id]),
          end=_viewer_array(cube_pos_w[env_id]),
      ...
      if viz.show_frame_origins:
        visualizer.add_sphere(
          center=_viewer_array(palm_center_pos_w[env_id]),
          radius=viz.origin_sphere_radius,
          color=viz.palm_center_origin_color,
          label=f"palm_center_origin_{env_id}",
        )
        visualizer.add_sphere(
          center=_viewer_array(cube_pos_w[env_id]),
          radius=viz.origin_sphere_radius,
          color=viz.cube_origin_color,
          label=f"cube_origin_{env_id}",
        )
      if viz.show_target_point:
        visualizer.add_sphere(
          center=_viewer_array(target_pos_w[env_id]),
          radius=viz.target_point_radius,
          color=viz.target_point_color,
          label=f"target_point_{env_id}",
        )
...
class HandCubeFrameVizCommandCfg(CommandTermCfg):
  ...
  class VizCfg:
    ...
    target_point_radius: float = 0.01
    ...
    target_point_color: tuple[float, float, float, float] = (0.1, 0.85, 1.0, 0.95)
